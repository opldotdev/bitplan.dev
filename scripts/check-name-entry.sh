...
agent-browser --session "$session" wait --fn 'document.querySelector("#shared-draft-title")?.value === "Naming flow check" && document.querySelector("button[type=submit]")?.disabled === false'
agent-browser --session "$session" set offline off
agent-browser --session "$session" focus '#shared-draft-title'
